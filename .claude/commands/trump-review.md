...
- `read_my_workspace(agent_name="trump")` — **NEW**: read your notes/, watchlist.md, and data/ folder. Anything in there is context for this hour. The user may have dropped a name into watchlist; research every active entry.
...
- `compute_all_models(agent_name="trump", symbol=...)` — auto-discovers and runs every model in `agents/trump/models/`. Returns top-level `error_count`, `errored_models`, `flat_count`, plus per-model `{version, result, error}` dict.
